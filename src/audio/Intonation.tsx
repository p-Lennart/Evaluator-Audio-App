...
function windowNumPerTs(timestamps: number[], lastWindow: number, sampleRate: number, hopLength: number): number[] {
    return timestamps.map(ts => {
        const windowNum = Math.floor((ts * sampleRate) / hopLength);
        if (windowNum > lastWindow) {
            console.error(`Given timestamp ${ts}s -> ${windowNum} is beyond audio length (last window ${lastWindow})`);
        }
...
function estimatePitchesAtTimestamps(
    ...
    console.log(`Audio length: ${audioPitches.length} windows, ~${audioPitches.length * hopLength / sampleRate}s?`)

    // Convert timestamp -> sample no., sample no. -> window no. using hop len
    const lastWindow = audioPitches.length - 1;
    const _windowNums = windowNumPerTs(timestampCol, lastWindow, sampleRate, hopLength);
    ...
    const _aggregateSizes = _windowNums.map((windowNum, idx) => {
        ...
        aggregateSize = Math.min(aggregateSize, (audioPitches.length - windowNum) - 1);
    ...
    const pitchEstimates = _windowNums.map((windowNum, idx) => {
        ...
        const rawAggregate = audioPitches.slice(windowNum, windowNum + aggregateSize)
...
export async function testIntonation(audioUri: string, tableUri: string, sr: number=44100) {
