 ...
 * @returns void
 */
export const musicXmlUploadWeb = (
  e: React.ChangeEvent<HTMLInputElement>,
  stateScores: string[],
  dispatch: Function,
) => {
  ...
  if (file) {
    ...
    reader.onload = (ev) => {
      ...
      if (!stateScores.includes(fileName)) {
        // Only add new score if the new uploaded score's name isn't already stored within scores
        const newScore = {
          filename: fileName,
 ...
 */
export const musicXmlUploadNative = async (
  stateScores: string[],
  dispatch: Function,
): Promise<void> => {
  try {
